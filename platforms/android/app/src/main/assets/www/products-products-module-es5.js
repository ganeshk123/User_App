    ...
    var ProductsPage = /*#__PURE__*/function () {
      ...
      _createClass(ProductsPage, [{
        ...
        value: function getProducts(limit, event) {
          ...
          this.api.post('stores/getByCity', stores).subscribe(function (stores) {
            if (stores && stores.status === 200 && stores.data && stores.data.length) {
              _this4.util.active_store = _toConsumableArray(new Set(stores.data.map(function (item) {
                return item.uid;
              })));
              var param = {
                id: _this4.id,
                limit: _this4.limit * 10,
                cid: localStorage.getItem('city')
              };

              _this4.api.post('products/getBySid', param).subscribe(function (data) {
                ...
                if (data && data.status === 200 && data.data && data.data.length) {
                  ...
                  _this4.products = products.filter(function (x) {
                    return x.status === '1' && _this4.util.active_store.includes(x.store_id);
                  });
